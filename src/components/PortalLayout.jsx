...
export default function PortalLayout({ active, onNavigate, items, title, children }) {
  ...
  const newAlerts = visibleAlerts?.filter((alert) => alert.status === 'New').length ?? 0
      ...
      <main className="portal-main">
        <header className="topbar">
          <div className="topbar-title">
            ...
            <div><span>{user?.portal === 'admin' ? 'Administrator portal' : 'Employee portal'}</span><strong>{title}</strong></div>
          </div>
          <div className="topbar-actions">
            ...
            <button className="icon-button notification-button" aria-label={`${user?.portal === 'admin' ? pendingApprovals + newAlerts : unreadNotifications} items need attention`} onClick={() => onNavigate(user?.portal === 'admin' ? 'action-center' : 'inbox')}>
              <Bell size={19} />
              {(user?.portal === 'admin' ? pendingApprovals + newAlerts : unreadNotifications) > 0 && (
                <span>{user?.portal === 'admin' ? pendingApprovals + newAlerts : unreadNotifications}</span>
              )}
            </button>
