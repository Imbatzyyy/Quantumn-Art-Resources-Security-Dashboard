...
describe('Employee self-service provider boundaries', () => {
  ...
  it('creates a private HR request with its classification and requested correction', async () => {
    ...
    expect(within(dialog).getByText('Time & Attendance')).toBeVisible()
  ...
  it('requires an explicit edit and confirmation before saving the permitted phone field', async () => {
    const user = userEvent.setup()
    ...
    const confirmation = screen.getByRole('dialog', { name: 'Confirm profile changes' })
    expect(within(confirmation).getByText(employee.phone!)).toBeVisible()
    expect(within(confirmation).getByText('+63 917 555 0101')).toBeVisible()
  ...
  it('retains the profile draft when saving fails so the employee can retry', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Save changes' }))
    const dialog = screen.getByRole('dialog', { name: 'Confirm profile changes' })
    ...
    expect(screen.getByLabelText('Phone number')).toHaveValue('+63 917 555 0102')
    await user.click(within(dialog).getByRole('button', { name: 'Confirm & save' }))
  ...
  it('rejects whitespace-only phone changes before showing confirmation', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Save changes' }))
    expect(notify).toHaveBeenCalledWith('Enter a phone number between 7 and 30 characters.', 'error')
    expect(updateEmployee).not.toHaveBeenCalled()
    expect(screen.queryByRole('dialog', { name: 'Confirm profile changes' })).not.toBeInTheDocument()
  })
